...
<p style="text-align:center; font-size:25px;padding:6px;">  
    ...
    <div class="rec"><br>
        ...
        <div class="rec_par">
        <strong><p style="text-align:right;"><?= lang('view.year_publication') ?> :</strong> <?= htmlspecialchars($rec['date_publi']) ?> </p>
        <strong><p style="text-align:right;"><?= lang('view.method_publication') ?> : </strong> <?= htmlspecialchars($rec['mode_publi']) ?> </p>
        <strong><p style="text-align:right;"><?= lang('view.several_written_narratives') ?> :</strong> <?= htmlspecialchars($rec['plrs_recits']) ?> </p>

    <div class='bouton-recit'>
      <?php if ($session->get('is_admin')) : ?>
            <td>
                <p><a href="<?= site_url('/modif_recit?esc='.esc(htmlspecialchars($rec['id_auteur'])).'&idR='.htmlspecialchars($rec['id_recit'])) ?>"><?= lang('recits.modify_button') ?></a></p>
            </td>

            <td>
                <p><a href="<?= site_url('Suppr/SupprRecit?esc='.esc(htmlspecialchars($rec['id_auteur'])).'&idR='.htmlspecialchars($rec['id_recit'])) ?>" onclick="return confirm('<?= lang('recits.delete_confirmation') ?>')"><?= lang('recits.delete_button') ?></a></p>
            </td>
            <td>
              <p><a href="/ajout_link"><?= lang('ajout_link.title') ?></a></p>
            </td>

        <?php endif; ?>
      </div>

    <strong><p><?= lang('view.name_slave') ?> :</strong> <?= htmlspecialchars($rec['nom_esc']) ?> </p>
    <strong><p><?= lang('view.type_narrative') ?> :</strong> <?= htmlspecialchars($rec['type_recit']) ?> </p>

    <strong><p><?= lang('view.date_birth') ?> :</strong> <?= htmlspecialchars($rec['naissance']) ?> </p>
    <strong><p><?= lang('view.location_publication') ?> : </strong> <?= htmlspecialchars($rec['lieu_publi']) ?> </p>
    <strong><p><?= lang('view.origins_parents') ?> :</strong> <?= htmlspecialchars($rec['origine_parents']) ?> </p>
    <strong><p><?= lang('view.name_writer') ?> :</strong> <?= htmlspecialchars($rec['scribe_editeur']) ?> </p>
    <strong><p><?= lang('view.additional_information') ?> :</strong> <?= htmlspecialchars($rec['particularites']) ?> </p>
</div>
...
<br>
<p><?= lang('view.link_narrative') ?> : <a href="<?= htmlspecialchars($rec['lien_recit']) ?>"><?= htmlspecialchars($rec['lien_recit']) ?></a></p>
